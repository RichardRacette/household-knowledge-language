...
def verify(root):
    ...
    if len(assertions) != len(profile["assertions"]) or len(decisions) != len(profile["decisions"]):
        raise ValueError("duplicate profile identity")
    for a in assertions.values():
        if a["ref"] not in claims or not set(a["source_refs"]) <= set(sources) or not set(a["derived_from"] + a["conflicts_with"]) <= set(assertions) or (a["supersedes"] and a["supersedes"] not in assertions):
            raise ValueError("broken assertion link")
        if a["decision_ref"]:
            d = decisions.get(a["decision_ref"], {})
            if d.get("target") != a["ref"] or d.get("scope") != "claim" or d.get("simulated") is not True:
                raise ValueError("broken simulated decision link")
    for d in decisions.values():
        if d.get("simulated") is not True or d.get("scope") != "claim" or d.get("target") not in assertions:
